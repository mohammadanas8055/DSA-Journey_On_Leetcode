...
public:
    int findNumbers(vector<int>& nums) {
        int count = 0; 
        // count stores kitne numbers me even number of digits hain
        ...
        for(int i = 0; i < nums.size(); i++){
            int digits = 0; 
            // Current number ke digits count karne ke liye

            int temp = nums[i];
            // Isse nums array destroy nahi hoga, current number ko dusre variable me store kar liya

            // while(temp != 0){ 
                // isme ye catch hai ki number agar 0 ho to digit 0 hi reh jayega, par yahan okay hai kyunki numbers positive hain
            //     digits++;
            //     temp /= 10;
            // }
            ...
            do{
                digits++;
                temp /= 10; 
                // Har baar number ko 10 se divide karne par
            ...
            if(digits % 2 == 0){
                count++; 
                // agar digits even -> count increase
            }
        }
        return count; 
        // final even-digit numbers ka count return
    }
};
