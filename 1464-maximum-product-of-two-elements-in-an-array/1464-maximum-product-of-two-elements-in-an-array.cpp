...
public:
    int maxProduct(vector<int>& nums) {
        ...
        // Pehle 2 elements me jo bada hai, use largest maan liya

        int secondLargest = min(nums[0], nums[1]);
        // Pehle 2 element me jo chhota hai, use second largest maan liya
        // Ab baaki elements ke saath in dono ko update karenge
        ...
        for(int i = 2; i < nums.size(); i++){
            if(nums[i] > largest){
                // Agar current element ab tak ka sabse bada mil gaya
                // to purana largest secondLargest ban jayega
            ...
            // 2. current is not largest, but can become second largest
                ...
                // Agar current element largest se chhota hai ya barabar hai
                // but secondLargest se bada hai
                // to ye naya secondLargest ban jayega
        ...
        // Problem ke according dono selected dono selected numbers me se 1 subtract karke multiply karna hai
    }
};
